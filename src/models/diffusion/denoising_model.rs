...
//
// 2. **Ordered parameter lists**: `params()` and `params_mut()` return
//    parameters in a fixed order. Gradients from `backward()` follow the same
//    order, so the optimizer can zip them together without knowing parameter
//    names.
//
// 3. **Named parameters**: `param_names()` returns human-readable names in the
//    same order as `params()`, enabling readable gradient-norm logging in
//    training binaries.
//
...
pub trait DenoisingModel {
    ...
    ///
    /// Used by the optimizer to initialise per-parameter state (e.g. Adam
    /// moment estimates). The order must be consistent with `backward()` output
    /// and `params_mut()`.
    fn params(&self) -> Vec<&Tensor>;

    /// Ordered mutable references to all trainable parameters.
    ///
    /// Used by the optimizer to apply weight updates in place.
    fn params_mut(&mut self) -> Vec<&mut Tensor>;

    /// Human-readable name for each parameter (same order as `params()`).
    ///
    /// Used by training binaries for gradient-norm logging, e.g.:
    ///   "w1 norm: 0.0042, b1 norm: 0.0001, w2 norm: 0.0031, b2 norm: 0.0002"
